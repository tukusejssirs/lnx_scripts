...
# created: 25 Feb 2018
# dependencies: awk bc case cut do done echo else esac exit fi for if printf sed seq then tr wget
# version: 1.1
...
case $1 in
	''|*[!0-9.-]*) echo 'Error: First argument must be a number, but it is "$1".' && exit 1 ;;
	*) sum=`printf "%1.4f" $1` ;;
esac
...
[ $# -lt 3 ] && {
	...
	echo "                  do not show warnings (you can still catch it using exit code, see below)"
	echo
